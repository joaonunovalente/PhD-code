...
def main():
    ...
    try:
        while True:
            ...
            registered_color_resized = cv2.resize(registered_color, (640, 480))
            depth_img_resized = cv2.resize(depth_img, (640, 480))

            # Display images using OpenCV
            cv2.imshow("Color", color_img_resized)
            cv2.imshow("Depth", depth_img_resized)  # Normalize for display
            cv2.imshow("Registered Color", registered_color_resized)
            ...
            cv2.imwrite(color_filename, registered_color_resized)
            cv2.imwrite(depth_filename, depth_img_resized)
